...
class AttachmentsUpdate
{
	...
	public function update_attachments_table()
	{
		...
		$db->setQuery($query);
		if (!$db->query()) {
			$errmsg = JText::_('ERROR_CHANGING_TABLE_TO_UTF8_UNICODE') . ' ' . $db->stderr() . ' (ERR 4)';
			JError::raiseError( 500, $errmsg );
			return false;
			}
		echo $indent .
			...
			if ( in_array( $old_name, $fields ) ) {
				...
				if (!$db->query()) {
					$errmsg = JText::sprintf('ERROR_RENAMING_FIELD_S_TO_S',
											 $old_name, $arr['new_name'] ) . ' ' . $db->stderr() . ' (ERR 6)';
					JError::raiseError(500, $errmsg);
					}
				$num_renamed_fields++;
				}
	...
	public function update_file_sizes()
	{
		...
		foreach ($IDs as $id) {
			...
			// Update the record
			if (!$attachment->store()) {
				$errmsg = $attachment->getError() . ' (ERR 23)';
				JError::raiseError(500, $errmsg);
				}
	...
	public function validate_urls()
	{
		...
		foreach ($IDs as $id) {
			...
			if ( $attachment->url_valid != $a->url_valid ) {
				...
				// Update the record
				if (!$attachment->store()) {
					$errmsg = $attachment->getError() . ' (ERR 26)';
					JError::raiseError(500, $errmsg);
					}
				$numUpdated++;
				}

...
class AttachmentsHelper
{
    ...
    public static function setupUploadDirectory($upload_dir, $secure)
    {
        ...
        $dirend_chars = DIRECTORY_SEPARATOR . '/';
        if (
            ( realpath(rtrim($upload_dir, $dirend_chars)) == realpath(JPATH_SITE) ) ||
    ...
    public static function uploadFile(&$attachment, &$parent, $attachment_id = false, $save_type = 'update')
    {
        ...
        $may_publish = $parent->userMayChangeAttachmentState(
    ...
    public static function downloadAttachment($id, $raw = 0, $popup = 0)
    {
        ...
        if ($attachment->uri_type == 'file') {
            ...
            if ($popup == 1) {
                ...
                // inline
                $content_disposition = 'attachment';
            } elseif ($download_mode != 'attachment') {
                $content_disposition = 'inline';
            }
            ...
            exit;
        } elseif ($attachment->uri_type == 'url') {
            // Note the download
            $model->incrementDownloadCount();
    ...
    public static function switchParent(
        ...
        $old_parent_id,
        $new_parent_id,
        $new_parent_type = null,
        $new_parent_entity = null
    ) {
     ...
     */
    public static function attachmentsListHTML(
        $parent_id,
        $parent_type,
        $parent_entity,
        $user_can_add,
        ...
        if ($total > 0) {
            ...
            if ($app->isClient('administrator')) {
                ...
                $alist = $controller->displayString(
                    $parent_id,
                    $parent_type,
                    $parent_entity,
                    null,
